    ...
    if 'usersessionid' in session:
        ...
        if request.method == 'POST':
            try:
                ...
                print(err)
            return render_template('brandsearch.html',session=session, form=mybrandsearchform, productdetails=productdetails)
...
def brand():
    ...
    if 'usersessionid' in session:
        ...
        if request.method == 'GET' and request.args.get('brandkey'):
            ...
            return render_template('brand.html',session=session, form=myform)
    else:
        return redirect(url_for('login'))
