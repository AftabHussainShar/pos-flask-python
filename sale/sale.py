    ...
    if 'usersessionid' in session:
        if request.method=='POST':
            ...
            for row in chunks:
                ...
                # Update the onhandquantity in inventory table
                sqlst = """UPDATE inventory 
                           SET onhandquantity = onhandquantity - %s 
                           WHERE warehousekey = %s AND productkey = %s"""
                myvalues = [quantity, session['warehousekey'], productkey]
                cursor.execute(sqlst, myvalues)
                # updatedqty = int(onhandqty[0]) - int(quantity)
        ...
        if request.method == 'GET' and request.args.get ('saleorderkey'):
            ...
            return render_template('sale2.html', session=session,form=mysaleform)
    else:
...
def getsoldproductquantity():
    ...
    if 'usersessionid' in session:
        if request.method=='POST':
            ...
            return render_template('refund2.html', session=session,form=myrefundform)
    else:
        return redirect (url_for('login'))
...
def searchsaleorders():
    ...
    if 'usersessionid' in session:
        ...
        if request.method=='POST':
            ...
            return render_template('saleordersearch.html',session=session, form=mysaleordersearchform ,productdetails=productdetails)
    ...
    if 'usersessionid' in session:
        ...
        if request.method=='POST':
            ...
            if request.form['btnview']=='btnprint':
                ...
                rendered =  render_template('reportprint.html', session=session,form=mydailysalereportform ,productdetails=productdetails)
            ...
            if request.form['btnview'] == 'btnview':
                return render_template('dailysalereport.html', session=session,form=mydailysalereportform, productdetails=productdetails)



        if request.method=='GET':
            return render_template('dailysalereport.html',session=session, form=mydailysalereportform )
    else:
        return redirect (url_for('login'))
