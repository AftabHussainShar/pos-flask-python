...
def grpo():
    ...
    if 'usersessionid' in session:
        ...
        if request.method == 'GET' and request.args.get('grpokey'):
            ...
            return render_template('grpomanual.html', session=session,form=mygrpoform)
    else:
...
# @grpo_bp.route('/savegrpo_', methods=['GET','POST'])
# def savegrpo():
#     mygrpoform = GRPOForm(request.form)
#     from app import mysql
#     if 'usersessionid' in session:
...
#             myvalues = [usersessionid]
#             cursor.execute(sqlst, myvalues)
#             grpokey = cursor.fetchone()[0]  # Accessing the first item in the tuple

#             for row in chunks:
#                 sqlst = "select productkey from product where barcode=%s"
#                 myvalues = [row[0]]
#                 cursor.execute(sqlst, myvalues)
#                 productkey = cursor.fetchone()[0]  # Accessing the first item in the tuple
...
def savegrpo():
    ...
    if 'usersessionid' in session:
        if request.method == 'POST':
            ...
            myvalues = [usersessionid]
            cursor.execute(sqlst, myvalues)
            grpokey = cursor.fetchone()[0]

            for row in chunks:
                sqlst = "select productkey from product where barcode=%s"
                myvalues = [row[0]]
                cursor.execute(sqlst, myvalues)
                productkey = cursor.fetchone()[0]
                ...
                myvalues = [quantity, warehousekey, productkey]
                cursor.execute(sqlst, myvalues)

            mysql.connection.commit()
            return jsonify(grpokey)
        else:
            return render_template('grpo.html', session=session,form=mygrpoform)
    else:
        return redirect(url_for('login'))
...
def searchgrpos():
    ...
    if 'usersessionid' in session:
        ...
        if request.method=='POST':
            ...
            return render_template('grposearch.html',session=session, form=mygrposearchform ,productdetails=productdetails)
