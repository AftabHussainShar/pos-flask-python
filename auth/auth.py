...
def main():
    if 'usersessionid' in session:
        ...
        cursor.execute("SELECT SUM(saleorderkey) FROM saleorder")
        all_orders_sum = cursor.fetchone()[0] 
        cursor.close()
        
        cursor = mysql.connection.cursor()
        cursor.execute("SELECT SUM(inventorykey) FROM inventory")
        all_products = cursor.fetchone()[0] 
        cursor.close()
        
        cursor = mysql.connection.cursor()
        cursor.execute("SELECT SUM(customerkey) FROM customer")
        all_customer = cursor.fetchone()[0] 
        cursor.close()
        
        cursor = mysql.connection.cursor()
        cursor.execute("SELECT SUM(refundorderkey) FROM refundorder")
        all_refundorder = cursor.fetchone()[0] 
        cursor.close()
        
        return render_template('index.html',session=session, all_orders_sum=all_orders_sum,all_products=all_products,all_customer=all_customer,all_refundorder=all_refundorder)
    else:
...
def login():
    ...
    if request.method=='POST':
        ...
        if (userdata):
            ...
            session['user_id'] = userdata[0]['userkey']
            cursor.close()
        ...
        return render_template('login.html', session=session,form=myloginform)
