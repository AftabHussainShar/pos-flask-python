    ...
    if 'usersessionid' in session:
        if request.method=='POST':
            try:
                ...
                print (err)
            return render_template('refundordersearch.html', session=session,form=myrefundordersearchform ,productdetails=productdetails)
    ...
    if 'usersessionid' in session:
        if request.method=='POST':
            ...
            for row in chunks:
                ...
                # Check if existinginventory contains any results
                if existinginventory:
                    # Access the first element of the first row of the result
                    updatedinventory = int(existinginventory[0][0]) + int(quantity)
                else:
                    # Handle the case where no results are returned
                    updatedinventory = int(quantity)  # Assuming quantity is the new inventory count
